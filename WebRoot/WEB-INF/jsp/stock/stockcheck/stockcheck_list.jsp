			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="stockcheck/list.do" method="post" name="Form" id="Form">
...
<!-- 								</td> -->
								...
								<td style="vertical-align:top;padding-left:2px">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-xs btn-success" onclick="add();">按仓库添加</a>
									<a class="btn btn-xs btn-success" onclick="addasgoods();">按商品添加</a>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.STOCKCHECK_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
...
<%-- 													<a class="btn btn-mini btn-danger"  style="height:26px;" onclick="fanshen('${var.STOCKCHECK_ID}');">反审</a> --%>
<%-- 													</c:if></c:if></c:if> --%>
<%-- 													<c:if test="${QX.InOrdershenpiApproval == 1 }"><c:if test="${var.BILLSTATUS != 2}"> --%>
<%-- 													<a class="btn btn-mini btn-success" style="height:26px;" onclick="shenpi('${var.STOCKCHECK_ID}');">审批</a> --%>
<%-- 													</c:if></c:if> --%>
													<a class="btn btn-xs btn-success" title="查看" onclick="view('${var.STOCKCHECK_ID}');">
														<i class="ace-icon fa fa-eye bigger-120" title="查看"></i>
													</a>
<%-- 													<c:if test="${QX.edit == 1 }"><c:if test="${var.BILLSTATUS != 2}"> --%>
<%-- 													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.STOCKCHECK_ID}');"> --%>
<!-- 														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i> -->
<!-- 													</a> -->
<%-- 													</c:if></c:if> --%>
													<c:if test="${QX.del == 1 }"><c:if test="${var.BILLSTATUS != 2}">
<%-- 													<a class="btn btn-xs btn-danger" onclick="del('${var.STOCKCHECK_ID}');"> --%>
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="view('${var.STOCKCHECK_ID}');" class="tooltip-success" data-rel="tooltip" title="查看">
																	<span class="green">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.STOCKCHECK_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.STOCKCHECK_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
...
<!-- 									<a class="btn btn-mini btn-success" onclick="shenpiAll('确定要批量审批选中的数据吗?');" title="批量审批" >批量审批</a> -->
	...
	<script type="text/javascript">
		...
		//修改
		function edit(Id){
			var url = 'stockcheck/goEdit.do?STOCKCHECK_ID='+Id;
		...
		//查看
		function view(Id){
			var url = 'stockcheck/goView.do?STOCKCHECK_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>stockcheck/excel.do?lastStart='+lastStart+'&lastEnd='+lastEnd;
		}
		...
		//跟踪记录
		function chaImg(STOCKCHECK_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="跟踪记录";
			 diag.URL = '<%=basePath%>customerimg/list.do?STOCKCHECK_ID='+STOCKCHECK_ID;
			 diag.Width = 800;
			 diag.Height = 650;
		...
		//消费记录
		function consume(STOCKCHECK_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="消费记录";
			 diag.URL = '<%=basePath%>outku/list.do?STOCKCHECK_ID='+STOCKCHECK_ID;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>stockcheck/delete.do?STOCKCHECK_ID="+Id+"&tm="+new Date().getTime();
		...
		function shenpi(Id){
			top.jzts();
			var url = "<%=basePath%>stockcheck/shenpi.do?STOCKCHECK_ID="+Id+"&tm="+new Date().getTime();
			$.get(url,function(data){
				tosearch();
			});
		}
		...
		function fanshen(Id){
			bootbox.confirm("确定要反审吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>stockcheck/fanshen.do?STOCKCHECK_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						tosearch();
					});
				}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要批量审批选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>stockcheck/shenpiAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>stockcheck/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>stockcheck/excel.do';
		}
	</script>
