			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="whallocate/list.do" method="post" name="Form" id="Form">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">调出仓</th>
									<th class="center">调入仓</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.WHALLOCATE_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center'>${var.BILLCODE}</td>
											<td class='center'>${var.LDATE}</td>
											<td class='center'>${var.OUTWH_NAME}</td>
											<td class='center'>${var.INWH_NAME}</td>
											<td class='center'>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.InOrderfanshenApproval == 1 }"><c:if test="${var.BILLSTATUS == 2}">
													<a class="btn btn-mini btn-danger"  style="height:26px;" onclick="fanshen('${var.WHALLOCATE_ID}');">反审</a>
													</c:if></c:if>
													<c:if test="${QX.InOrdershenpiApproval == 1 }"><c:if test="${var.BILLSTATUS != 2}">
													<a class="btn btn-mini btn-success" style="height:26px;" onclick="shenpi('${var.WHALLOCATE_ID}');">审批</a>
													</c:if></c:if>
													<a class="btn btn-xs btn-success" title="查看" onclick="view('${var.WHALLOCATE_ID}');">
														<i class="ace-icon fa fa-eye bigger-120" title="查看"></i>
													</a>
<%-- 													<c:if test="${QX.edit == 1 }"><c:if test="${var.BILLSTATUS != 2}"> --%>
<%-- 													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.WHALLOCATE_ID}');"> --%>
<!-- 														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i> -->
<!-- 													</a> -->
<%-- 													</c:if></c:if> --%>
													<c:if test="${QX.del == 1 }"><c:if test="${var.BILLSTATUS != 2}">
													<a class="btn btn-xs btn-danger" onclick="del('${var.WHALLOCATE_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="view('${var.WHALLOCATE_ID}');" class="tooltip-success" data-rel="tooltip" title="查看">
																	<span class="green">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.WHALLOCATE_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.WHALLOCATE_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			document.forms.actionForm.action="whallocate/goAdd.do";
	        document.forms.actionForm.submit();
			//siMenu('z190','lm180','添加进货单','whallocate/goAdd.do');
		}
		//修改
		function edit(Id){
			var url = 'whallocate/goEdit.do?WHALLOCATE_ID='+Id;
		...
		//查看
		function view(Id){
			var url = 'whallocate/goView.do?WHALLOCATE_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>whallocate/excel.do?lastStart='+lastStart+'&lastEnd='+lastEnd;
		}
		//检索
		function tosearch(){
			top.jzts();
		...
		//跟踪记录
		function chaImg(WHALLOCATE_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="跟踪记录";
			 diag.URL = '<%=basePath%>customerimg/list.do?WHALLOCATE_ID='+WHALLOCATE_ID;
			 diag.Width = 800;
			 diag.Height = 650;
		...
		//消费记录
		function consume(WHALLOCATE_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="消费记录";
			 diag.URL = '<%=basePath%>outku/list.do?WHALLOCATE_ID='+WHALLOCATE_ID;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>whallocate/delete.do?WHALLOCATE_ID="+Id+"&tm="+new Date().getTime();
		...
		function shenpi(Id){
			top.jzts();
			var url = "<%=basePath%>whallocate/shenpi.do?WHALLOCATE_ID="+Id+"&tm="+new Date().getTime();
			$.get(url,function(data){
				tosearch();
			});
		}
		...
		function fanshen(Id){
			bootbox.confirm("确定要反审吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>whallocate/fanshen.do?WHALLOCATE_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						tosearch();
					});
				}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要批量审批选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>whallocate/shenpiAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>whallocate/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
