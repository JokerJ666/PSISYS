...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="resalebill/list.do" method="post" name="Form" id="Form">
								...
								<c:choose>
									<c:when test="${not empty varList}">
										...
										<c:forEach items="${varList}" var="var" varStatus="vs">
											<tr id ="${var.SALEBILL_ID}"  onclick="clickaction('${var.SALEBILL_ID}');">
												<td class='center' id='checkbox${var.SALEBILL_ID }'>
												...
												<td class="center">
													...
													<div class="hidden-sm hidden-xs btn-group">
														...
														<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.SALEBILL_ID}','${var.BILLSTATUS}');">
															<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
														</a>
														</c:if>
														<c:if test="${QX.del == 1 }">
														<a class="btn btn-xs btn-danger" onclick="del('${var.SALEBILL_ID}','${var.BILLSTATUS}');">
													...
													<div class="hidden-md hidden-lg">
														<div class="inline pos-rel">
															...
															<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
																...
																<c:if test="${QX.edit == 1 }">
																<li>
																	<a style="cursor:pointer;" onclick="edit('${var.SALEBILL_ID}','${var.BILLSTATUS}');" class="tooltip-success" data-rel="tooltip" title="修改">
																		<span class="green">
																...
																<c:if test="${QX.del == 1 }">
																<li>
																	<a style="cursor:pointer;" onclick="del('${var.SALEBILL_ID}','${var.BILLSTATUS} }');" class="tooltip-error" data-rel="tooltip" title="删除">
																		<span class="red">
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			document.forms.actionForm.action="resalebill/goAdd.do";
	        document.forms.actionForm.submit();
			//siMenu('z190','lm180','添加进货单','salebill/goAdd.do');
		}
		//修改
		function edit(Id,BILLSTATUS){
			...
			var url = 'resalebill/goEdit.do?SALEBILL_ID='+Id;
		...
		//查看
		function view(Id){
			var url = 'resalebill/goView.do?SALEBILL_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
		...
		function clickaction(SALEBILL_ID){
			$.ajax({
		        method:'POST',
		        url:'salebill/salebillListBody',
		        data:{'SALEBILL_ID':SALEBILL_ID},
		        dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>resalebill/excel.do?lastStart='+lastStart+'&lastEnd='+lastEnd;
		}
		...
		function del(Id,BILLSTATUS){
			...
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>resalebill/delete.do?SALEBILL_ID="+Id+"&tm="+new Date().getTime();
		...
		function shenpi(Id,BILLSTATUS){
			...
			if($("#"+Id).text().match(RegExp(/已审核/)) ){
			 ...
			 $.ajax({
					type: "POST",
					url: '<%=basePath%>resalebill/shenpi.do?tm='+new Date().getTime(),
			    	data: {SALEBILL_ID:Id},
					dataType:'json',
		...
		/* 	if($("#"+Id).text().match(RegExp(/作废/)) ){
			...
			bootbox.confirm("确定要反审吗?", function(result) {
				if(result) {
					...
					var url = "<%=basePath%>resalebill/fanshen.do?SALEBILL_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						tosearch();
					});
				}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要批量审批选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>salebill/shenpiAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>salebill/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>salebill/excel.do';
		}
	</script>
