			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="reinorder/list.do" method="post" name="Form" id="Form">
...
<%-- 								<td style="padding-left:2px;"><input class="span10 date-picker" name="lastEnd" name="lastEnd"  value="${pd.lastEnd }" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:88px;" placeholder="结束日期" title="结束日期"/></td> --%>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			document.forms.actionForm.action="reinorder/goAdd.do";
		...
		function edit(Id,BILLSTATUS){
			...
			var url = 'reinorder/goEdit.do?INORDER_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
			//siMenu('z191','lm181','修改退货单',url);
		}
		//查看
		function view(Id){
			var url = 'reinorder/goView.do?INORDER_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
		...
		function del(Id,BILLSTATUS,ISSETTLEMENTED){
			...
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>reinorder/delete.do?INORDER_ID="+Id+"&tm="+new Date().getTime();
		...
		function shenpi(Id,BILLSTATUS){
			...
			top.jzts();
			var url = "<%=basePath%>reinorder/shenpi.do?INORDER_ID="+Id+"&tm="+new Date().getTime();
			$.get(url,function(data){
				tosearch();
			});
		}
		...
		function fanshen(Id,ISSETTLEMENTED,BILLSTATUS){
			...
			bootbox.confirm("确定要反审吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>reinorder/fanshen.do?INORDER_ID="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						tosearch();
					});
				}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要批量审批选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>reinorder/shenpiAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>reinorder/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
								//beforeSend: validateData,
								cache: false,
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>reinorder/excel.do';
		}
	</script>
