...
<body class="no-skin">
	...
	$(top.hangge());
		var salebillitemmap = new Map(); //进货单每个子项目的html代码
        ...
        var settledsalebillmap = new Map();  //结过算的进货单主键 
			            	...
			            	for(var i = 0; i < res.varList.length; i++){
			            		...
			            		if(res.QX.cha == 1){
				            		...
				            		if(res.QX.SUPPSETBILLSET == 1 ){
										...
										html += "		<a class='btn btn-xs btn-success' title='结算' id='settleOnInorder' onclick=\"settleone('"+res.varList[i].SALEBILL_ID+"','"+res.varList[i].BILLTYPE+"');\"> ";
										html += "			<i class='ace-icon fa fa-eye bigger-120' title='结算'></i> ";
										html += "		</a> ";
				            		}
									if(res.QX.del == 1){
										html += "		<a class='btn btn-xs btn-danger' id='delInorder' onclick=\"del('"+res.varList[i].SALEBILL_ID+"','"+res.varList[i].BILLTYPE+"');\" > ";
									...
									if(res.QX.SUPPSETBILLSET == 1){
										html += "				<li> ";
										html += "					<a style='cursor:pointer;' id='settleOnInorder' onclick=\"settleone('"+res.varList[i].SALEBILL_ID+"','"+res.varList[i].BILLTYPE+"');\" class='tooltip-success' data-rel='tooltip' title='结算'> ";
									...
									if(res.QX.del == 1){
										html += "				<li> ";
										html += "					<a style='cursor:pointer;' id='delInorder' onclick='del('"+res.varList[i].SALEBILL_ID+"','"+res.varList[i].BILLTYPE+"');' class='tooltip-error' data-rel='tooltip' title='删除'> ";
										html += "						<span class='red'> ";
				            		...
				            		unpaidamountmap.set(res.varList[i].SALEBILL_ID+","+res.varList[i].BILLTYPE,res.varList[i].UNPAIDAMOUNT);  //未付金额
				            		firstmap.set(res.varList[i].SALEBILL_ID+","+res.varList[i].BILLTYPE,res.varList[i].UNPAIDAMOUNT);  //未付金额
				            		paidamoutmap.set(res.varList[i].SALEBILL_ID+","+res.varList[i].BILLTYPE,res.varList[i].PAIDAMOUNT);  //已付金额
		...
		function changepay(){
			if($("#select_paymethod").val() == '0' && $("#select_customersetbill").val() == '0'){
            	...
            	if(parseInt($("#PAYABLEAMOUNT").val())+customerhaspaidamount < parseInt($("#PAYMENTAMOUNT").val())){
	            	...
	            	if(customerhaspaidamount==0){
	            		$("#PAYMENTAMOUNT").val(parseFloat($("#PAYABLEAMOUNT").val())+customerhaspaidamount)
	            	}else{
	            		$("#PAYMENTAMOUNT").val(parseFloat($("#PAYABLEAMOUNT").val()));
	            	}
	            }
	            if(parseFloat($("#PAYMENTAMOUNT").val()) < customerhaspaidamount && customerhaspaidamount != 0.0){
	            	$("#PAYMENTAMOUNT").tips({
						side : 1,
						msg : "当前付款总额至少为已结算销售单的额度",
						bg : '#FF5080',
						time : 3
					});
		...
		function settleone(Id,type){
			...
			if(settledsalebillmap.get(ids) != 1){
				...
				var canpaidamount = thispayamount-customerhaspaidamount;  //可进行进货单结算的余额=结算单付款金额-已结算进货单总额 
				if(canpaidamount == 0.0){
					$("#PAYMENTAMOUNT").tips({
						side : 1,
						msg : "当前实付金额已用于结算了，请增加额度，才可进行接下来的销售单结算",
						bg : '#FF5080',
						time : 5
					});
				}else {
					...
					var nosettle = 0;
					if(type=='2'){ //如果此单为进货单
						if(canpaidamount < unpaid){ //如果可以支付的金额小于未付金额
					...
					}else{ //为退货单,本来就在总金额那里已经扣去了
						bootbox.confirm("确定要结算该进货单吗?", function(result) {
							if(result) {
								customerhaspaidamount += nextsalebillcanpaid;
				        		$("#"+Id+" #UNPAIDAMOUNT").html(0);
				        	    $("#"+Id+" #PAIDAMOUNT").html("-"+unpaid);
				        		$("#"+Id+" #THISPAY").html("-"+unpaid);
				        		$("#"+Id+" #ISSETTLEMENTED").html(2);
				        		settledsalebillmap.set(ids,1);//完全支付
				        		paidamoutmap.set(ids,"-"+unpaid);
								...
								for(var i=0;i < document.getElementsByName('ids').length;i++){
									if(document.getElementsByName('ids')[i].checked){									
										  ...
										  if(type=='2'){
											  ...
											  canpaidamount -= 0;
											  $("#"+id+" #UNPAIDAMOUNT").html(0);
								        	  $("#"+id+" #PAIDAMOUNT").html("-"+allamountmap.get(ids));
								        	  $("#"+id+" #THISPAY").html("-"+allamountmap.get(ids));
								        	  $("#"+id+" #ISSETTLEMENTED").html(2);
								        	  paidamoutmap.set(ids,"-"+allamountmap.get(ids));
								              unpaidamountmap.set(ids,0);
								              settledsalebillmap.set(ids,1);
...
/* 								var j=0;
		...
		function savecustomersetbill(){
			...
			var flag=1;//用来标明退货单是不是第一个，是的就不用加 ","
