			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="payment/listOrderSaleByPayment.do" method="post" name="Form" id="Form">
							<table style="margin-top:5px;">
								<tr>
									...
									<td>
										<select class="chosen-select form-control" name="PAYMETHOD_ID" id="PAYMETHOD_ID" data-placeholder="选择支付方式" style="vertical-align:top;width:98%;" >
											<option value="" >选择支付方式</option>
										   	<c:forEach items="${paymentlist}" var="var">
												<option value="${var.PAYMETHOD_ID }"<c:if test="${var.PAYMETHOD_ID==paymentId}">selected</c:if> >${var.PAYMETHODNAME }</option>  
											</c:forEach>
										</select>
									</td>
									<c:if test="${QX.cha == 1 }">
							...
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
								<thead>
									<tr>
										...
										<c:if test="${varlist[0].BILLTYPE=='1'}">
											<th class="center">供应商</th>
										</c:if>
										<c:if test="${varlist[0].BILLTYPE=='2'}">
									...
									<c:when test="${not empty varlist}">
										...
										<c:forEach items="${varlist}" var="var" varStatus="vs">
											<tr >
												...
												<c:if test="${var.PAYMETHODNAME==null}">
													<td class='center'>多种方式支付</td>			
												</c:if>				
												<c:if test="${var.PAYMETHODNAME!=null}">
