...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.PAYMETHOD_NAME}</td>
											<td class='center'>${var.PAYORIGIN}</td>
											<td class='center'>${var.PAYAMOUNT}</td>
											<td class='center'>${var.TICKETNUM}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-mini btn-danger"  style="height:26px;" onclick="fanshen('${var.EXPENSE_ID}');">反审</a>
													</c:if></c:if></c:if>
													<c:if test="${QX.InOrdershenpiApproval == 1 }"><c:if test="${var.BILLSTATUS != 2}">
													<a class="btn btn-mini btn-success" style="height:26px;" onclick="shenpi('${var.EXPENSE_ID}');">审批</a>
													</c:if></c:if> --%>
													<a class="btn btn-xs btn-success" title="查看" onclick="view('${var.EXPENSE_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<li>
																<a style="cursor:pointer;" onclick="view('${var.EXPENSE_ID}');" class="tooltip-success" data-rel="tooltip" title="查看">
																	<span class="green">
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			document.forms.actionForm.action="expense/goAdd.do";
	        document.forms.actionForm.submit();
			//siMenu('z190','lm180','添加进货单','expense/goAdd.do');
		}
		//修改
		function edit(Id){
			var url = 'expense/goEdit.do?EXPENSE_ID='+Id;
		...
		//查看
		function view(Id){
			var url = 'expense/goView.do?EXPENSE_ID='+Id;
			document.forms.actionForm.action=url;
	        document.forms.actionForm.submit();
		...
		//跟踪记录
		function chaImg(EXPENSE_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="跟踪记录";
			 diag.URL = '<%=basePath%>customerimg/list.do?EXPENSE_ID='+EXPENSE_ID;
			 diag.Width = 800;
			 diag.Height = 650;
		...
		//消费记录
		function consume(EXPENSE_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="消费记录";
			 diag.URL = '<%=basePath%>outku/list.do?EXPENSE_ID='+EXPENSE_ID;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = false;			//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
