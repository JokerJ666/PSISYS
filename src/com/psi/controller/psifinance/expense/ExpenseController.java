...
//		mv.addObject("varListL", varListL);
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出费用开支单到excel");
		...
		List<PageData> varOList = expenseService.listAllToExcel(pd);
		
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("PAYMETHOD_NAME"));	    //1
			vpd.put("var2", varOList.get(i).getString("PAYORIGIN"));	    //2
			vpd.put("var3", String.valueOf(varOList.get(i).get("PAYAMOUNT")));	//3
			vpd.put("var4", varOList.get(i).getString("TICKETNUM"));	    //4
			vpd.put("var5", varOList.get(i).getString("PSI_NAME"));	//5
			vpd.put("var6", varOList.get(i).getString("NOTE"));	    //6
			varList.add(vpd);
		}
		dataMap.put("title", "费用开支单");
