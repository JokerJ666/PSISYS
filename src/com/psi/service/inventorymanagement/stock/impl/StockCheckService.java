...
public class StockCheckService implements StockCheckManager{
	...
	public void save(PageData pd)throws Exception{
		String[] strs = productBillCodeUtil.getBillCode(Const.BILLCODE_STOCKCHECK_PFIX); //获取该编号类型的最大编号
		...
		if(strs[1] == null){ //新增
			...
			psiBillCode.setCodeType(Const.BILLCODE_STOCKCHECK_PFIX);
	 ...
	 * @throws Exception 
	 */
	public void updateStock(String WAREHOUSE_ID, String GOODCODE_ID, String CHENKNUM, String LOSSNUM) throws Exception {
		PageData head = new PageData();
		//==================更新库存====================
		//把商品的编号加入到查询条件
		head.put("GOOD_ID", GOODCODE_ID);
		head.put("WAREHOUSE_ID", WAREHOUSE_ID);
		...
		head.put("STOCKNUM", (Integer)aGoods.get("STOCKNUM") + Integer.valueOf(LOSSNUM));
...
//			dao.save("StockMapper.save", head);
	...
	public void delete(PageData pd)throws Exception{
		dao.update("StockCheckMapper.delete", pd);
		dao.update("StockCheckBodyMapper.delete", pd);
	}
	/**
	 * 批量结算销售单
	 */
	...
	public void settleSalebills(List<PageData> inorderandbodylist) throws Exception {
		for(int i = 0; i < inorderandbodylist.size(); i++) {
			...
			dao.update("StockCheckMapper.updateForSettle", pd);
		}
	...
	public List<PageData> listForByCustomersetId(PageData pd) throws Exception {
		List<PageData> list = (List<PageData>)dao.findForList("StockCheckMapper.listForByCustomersetId", pd);
		for(int i = 0; i < list.size(); i++) {
			list.get(i).put("bodylist", (List<PageData>)dao.findForList("StockCheckBodyMapper.findById", list.get(i)));
		}
		return list;
	}
	...
	public void edit(PageData pd)throws Exception{
		...
		dao.update("StockCheckMapper.edit", pd);
	}
	...
	public List<PageData> list(Page page)throws Exception{
		return (List<PageData>)dao.findForList("StockCheckMapper.datalistPage", page);
	}
	/**
	 * 结算单里获取销售单列表
	 * @param page
	...
	public List<PageData> listForCustomer(PageData pd)throws Exception{
		...
		if(SALEBILL_IDS != "" && !"".equals(SALEBILL_IDS) && SALEBILL_IDS != null) {
			...
			for(int i = 0 ; i < sbids.length; i++) {
				...
				list.add((PageData)dao.findForObject("StockCheckMapper.findBySalebillId", salebillandbody));
			}
		}
		return list;
	}
	...
	public List<PageData> listForCustomerAdd(Page page)throws Exception{
		List<PageData> list = (List<PageData>)dao.findForList("StockCheckMapper.datalistPageByCustomerset", page);
		return list;
	}
	/**
	...
	public PageData findAllById(PageData pd)throws Exception{
		PageData result =  (PageData)dao.findForObject("StockCheckMapper.findAllById", pd);
		List<PageData> list = (List<PageData>)dao.findForList("StockCheckBodyMapper.findInBodyById", pd);
	...
	public List<PageData> listAll(PageData pd)throws Exception{
		return (List<PageData>)dao.findForList("StockCheckMapper.listAll", pd);
	}
	...
	public PageData findById(PageData pd)throws Exception{
		PageData result =  (PageData)dao.findForObject("StockCheckMapper.findById", pd);
		pd.put("WAREHOUSE_ID", result.get("WAREHOUSE_ID"));
		result.put("goodslist", (List<PageData>)dao.findForList("StockCheckBodyMapper.findById", pd));
		return result;
	}
...
//		dao.update("StockCheckMapper.deleteAll", ArrayDATA_IDS);
//	}
	...
	public void retrialInorder(PageData pd) throws Exception {
		dao.update("StockCheckMapper.retrialInorder", pd);
	}
	...
	public PageData settleOneSalebill(PageData pd) throws Exception {
		...
		dao.update("StockCheckMapper.settleOneSalebill", pd);
		return pd;
	}
	...
	public List<HashMap> settleAllSalebill(PageData pd) throws Exception {
		...
		if(createArr!="" || createArr != null){
			...
			for(int i=0; i < pa.size(); i++) {
				...
				list.add(map);
				dao.update("StockCheckMapper.settleOneSalebill", map);
			}
		}
		return list;
	}
	...
	public void updateshenpi(PageData pd) throws Exception {
		//把销售单的状态改为已审核
		dao.update("StockCheckMapper.shenpi", pd);
		//获取销售单表头数据
		PageData head =  (PageData)dao.findForObject("StockCheckMapper.findById", pd);
		//通过销售单ID获取该销售单的商品信息
		List<PageData> goodslist = (List<PageData>)dao.findForList("StockCheckBodyMapper.findById", pd);
...
//				dao.save("StockMapper.save", head);
	...
	public void updatefanshen(PageData pd) throws Exception {
		//把销售单的状态改为未审核
		dao.update("StockCheckMapper.fanshen", pd);
		//获取销售单表头数据
		PageData head =  (PageData)dao.findForObject("StockCheckMapper.findById", pd);
		//通过销售单ID获取该销售单的商品信息
		List<PageData> goodslist = (List<PageData>)dao.findForList("StockCheckBodyMapper.findById", pd);
...
//		jdbcTempUtil.shenpiAllOrder(idstr.toString().substring(0,idstr.toString().length()-1), (String)Jurisdiction.getSession().getAttribute(Const.SESSION_PK_SOBOOKS), "psi_inorder", "INORDER_ID");
	...
	public void editFromCustomer(PageData pd) throws Exception {
		dao.update("StockCheckMapper.editFromCustomer", pd);
	}
